...
class PermissionConverter {
    ...
    companion object {
        ...
        private val PERMISSION_NAME_MAP = mutableMapOf<String, Int>()

        /** 权限描述映射（为了适配多语种，这里存储的是 StringId，而不是 String） */
        private val PERMISSION_DESCRIPTION_MAP = mutableMapOf<Int, Int>()
        ...
        init {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                PERMISSION_DESCRIPTION_MAP[R.string.common_permission_nearby_devices] = R.string.common_permission_nearby_devices_description
            } else {
                // 需要填充文案：蓝牙权限描述
                PERMISSION_DESCRIPTION_MAP[R.string.common_permission_nearby_devices] = R.string.common_permission_nearby_devices_description
            }
            ...
            val sensorsPermissionNameStringId = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA) {
            ...
            val bodySensorsBackgroundPermissionNameStringId = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA) {
            ...
            val activityRecognitionPermissionNameStringId = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
        ...
        fun getNickNamesByPermissions(context: Context, permissions: List<IPermission>): String {
            ...
            return if (builder.isEmpty()) {
                ...
                context.getString(R.string.common_permission_unknown)
            } else {
                builder.toString()
            }
        }
        ...
        fun getNickNameListByPermissions(
            ...
            val permissionNickNameList = mutableListOf<String>()
            for (permission in permissions) {
                // 如果当前设置了过滤高版本权限，并且这个权限是高版本系统才出现的权限，则不继续往下执行
                // 避免出现在低版本上面执行拒绝权限后，连带高版本的名称也一起显示出来，但是在低版本上面是没有这个权限的
                if (filterHighVersionPermissions && permission.fromAndroidVersion > Build.VERSION.SDK_INT) {
                    continue
                }
        ...
        fun getNickNameByPermission(context: Context, permission: IPermission): String {
            ...
            return if (permissionNameStringId == null || permissionNameStringId == 0) {
                ""
            } else {
                context.getString(permissionNameStringId)
            }
        }
        ...
        fun getPermissionNickNameStringId(context: Context, permission: IPermission): Int? {
            ...
            val permissionGroup = permission.permissionGroup
