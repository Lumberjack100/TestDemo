...
class CRC16 {
    companion object {
         ...
         * @return 校验码
         */
        fun getCRC16(data: String): String {
            val processedData = data.replace(" ", "")
            val len = processedData.length
            if (len % 2 != 0) {
                return "0000"
            }
            val num = len / 2
            ...
            for (i in 0 until num) {
                val value = Integer.valueOf(processedData.substring(i * 2, 2 * (i + 1)), 16)
                para[i] = value.toByte()
            }
            return getCRC(para)
        }
        ...
        private fun getCRC(bytes: ByteArray): String {
            ...
            //结果转换为16进制
            var result = Integer.toHexString(crc).uppercase(Locale.ROOT)
            if (result.length != 4) {
