...
import com.shmedo.mcloudapp.databinding.FragmentMl101Rs485ConfigBinding
import com.shmedo.mcloudapp.extensions.nav
import com.shmedo.mcloudapp.extensions.registerOnBackPressedDispatcher
import com.shmedo.mcloudapp.ui.page.device.OptimizedBaseIOTDeviceFragment
import com.shmedo.mcloudapp.ui.viewmodel.state.ML101RS485ConfigViewModel
 ...
 *
 * 功能说明：
 * 配置 ML101 设备的 RS485 串口通信参数，包括：
 * - 开关：控制 RS485 功能的启用/禁用
 * - 功能选择：关闭输出/日志输出/数据透传
 ...
 * - 设置参数: $cmd=md_cfgextserialparam&method=1&rs485_sw=...&rs485_mode=...
 *
 * 注意事项：
 * 1. 开关值的含义与直觉相反：0-开, 1-关
 * 2. 当开关关闭时，其他配置项应禁用
 */
class ML101RS485ConfigFragment : OptimizedBaseIOTDeviceFragment() {

    private lateinit var binding: FragmentMl101Rs485ConfigBinding
    private val toolbarViewModel: ToolbarViewModel by viewModels()
    private val mStates: ML101RS485ConfigViewModel by viewModels()
    ...
    override fun getDataBindingConfig(): DataBindingConfig {
        return DataBindingConfig(R.layout.fragment_ml101_rs485_config, BR.stateVM, mStates)
            .addBindingParam(BR.toolbarVM, toolbarViewModel)
            .addBindingParam(BR.click, ClickProxy())
    }

    override fun initView(savedInstanceState: Bundle?) {
        binding = getBinding() as FragmentMl101Rs485ConfigBinding
        binding.llToolbar.toolbar.title = "RS485配置"
    ...
    /**
     * 保存 RS485 配置
     * 发送设置指令: $cmd=md_cfgextserialparam&method=1&rs485_sw=...
     */
    ...
    private fun buildSaveCommands(): List<String> {
        ...
        // 创建 RS485 配置实体
        val entity = ML101ExtSerialParamEntity.createRS485Entity(
            sw = swValue,
            mode = modeValue,
    ...
    private fun handleQueryResponse(cmdStr: String) {
        ...
        when (result) {
            is IOTCommandResult.Failure -> {
                val errMsg = "查询RS485参数出错: ${result.message}"
    ...
    private fun handleSaveResponse(cmdStr: String) {
        when (val result = iotParseManager.parse<CommonSettingCmdResult>(cmdStr)) {
            is IOTCommandResult.Failure -> {
                val errMsg = "RS485参数保存出错: ${result.message}"
                handleFailureResult(errMsg, isMessageDialog = true)
            }
    ...
    private fun initParamData(data: ML101ExtSerialParamInfo) {
        try {
            // 开关：0-开, 1-关，需要转换
            mStates.isOpened.set(data.isRs485Enabled())

            // 功能模式
            val modeIndex = data.rs485_mode
            if (modeIndex in functionModeDisplayList.indices) {
                mStates.functionMode.set(functionModeDisplayList[modeIndex])
            }

            // 波特率
            mStates.baudRate.set(data.rs485_baud.toString())

            // 数据位
            mStates.dataBit.set(data.rs485_dataBit.toString())

            // 校验位：需要转换
            val parityIndex = parityBitValueList.indexOf(data.rs485_paritybit.uppercase())
            if (parityIndex in parityBitDisplayList.indices) {
                mStates.parityBit.set(parityBitDisplayList[parityIndex])
            }

            // 停止位
            mStates.stopBit.set(data.rs485_stopbit)

            // 保存初始状态
            mStates.saveInitialState()
        } catch (e: Exception) {
            Timber.e(e, "初始化RS485参数数据出错")
        }
    }
