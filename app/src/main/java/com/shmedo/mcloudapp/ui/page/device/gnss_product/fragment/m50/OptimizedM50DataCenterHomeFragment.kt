 ...
 * 4. 支持4G和蓝牙两种通讯方式
 */
class OptimizedM50DataCenterHomeFragment : OptimizedBaseDataCenterHomeFragment() {
    ...
    override fun queryData() {
        ...
        for (i in 1..centerNum) {
            ...
            commands.add(IOTCommandUtil.getCommand(IOTCommandType.MD_GET_DATA_CENTER_STATUS, entity))
        }

        executeDataCenterCommands(
            commands = commands,
            showLoading = false, // 刷新时不显示加载对话框
    ...
    override fun initSaveCommand() {
        ...
        val entity = DataReportTypeEntity(
            type = reportMethodList.indexOf(dataReportingPeriodItem.getReportMethodStr()).toString(),
    ...
    inner class M50ClickProxy {
        ...
        fun onReportingMethodClick() {
            val selectedIndex = reportMethodList.indexOf(dataReportingPeriodItem.getReportMethodStr())
        ...
        fun onReportingStartTimeClick() {
            val selectedIndex = reportStartTimeList.indexOf(dataReportingPeriodItem.getReportStartTimeHourStr())
            XPopup.setPrimaryColor(ColorUtils.getColor(R.color.colorPrimary))
            XPopup.Builder(context)
                .maxHeight((ScreenUtils.getAppScreenHeight() * 0.6f).toInt())
                .isDestroyOnDismiss(true)
