...
import com.shmedo.mcloudapp.databinding.FragmentLb20sGatewayInfoBinding
...
import com.shmedo.mcloudapp.device.viewmodel.state.LB20SGatewayInfoViewModel
...
class LB20SGatewayInfoFragment : BaseIOTDeviceFragment() {
    private lateinit var binding: FragmentLb20sGatewayInfoBinding
    private lateinit var mStates: LB20SGatewayInfoViewModel
    ...
    override fun getDataBindingConfig(): DataBindingConfig {
        return DataBindingConfig(R.layout.fragment_lb20s_gateway_info, BR.stateVM, mStates)
    }

    override fun initView(savedInstanceState: Bundle?) {
        binding = getBinding() as FragmentLb20sGatewayInfoBinding
        initRefresh()
        initAdapter()
    }
    ...
    private fun initAdapter() {
        binding.recyclerview.linear().setup { rv ->
            ...
            addType<DeviceStatusInfoBasicItem>(R.layout.item_device_status_info_signal_item)
            R.id.item.onClick {
                when (itemViewType) {
                ...
                groupList.add(
                    DeviceStatusInfoBasicItem(
                        ...
                        value = formatDoubleValue(
                            stateInfo.solar_volt.toDoubleOrNull(),
                            decimalFormat,
                            "0"
                        ) + "V",
            ...
            if (stateInfo.battery_volt != IOTConstants.NULL_KEY) {
                ...
                groupList.add(
                    DeviceStatusInfoBasicItem(
                        name = "蓄电池电压",
                        value = formatDoubleValue(
                            stateInfo.battery_volt.toDoubleOrNull(),
                            decimalFormat,
                            "0"
                        ) + "V",
                        colorRes = if (tempValue <= 5) ColorUtils.getColor(R.color.device_offline_platform) else ColorUtils.getColor(
