...
public class MagnificationView extends FrameLayout {
    ...
    @BindView(R.id.ll_ropeLength)
    View ropeLengthView;//测段长
    ...
    public void initData(VmsTerminalSensorInfo sensorInfo) {
        ...
        //钻孔测斜仪
        if (monitoringType == MonitoringType.BOREHOLE_INCLINOMETER) {
            ropeLengthView.setVisibility(View.VISIBLE);

        } else {
            ropeLengthView.setVisibility(View.GONE);
        }
    ...
    public void setVisibilityBySensorType(String sensorName) {
        MonitoringType monitoringType = MonitoringType.valueByDesc(sensorName);
        //钻孔测斜仪
        if (monitoringType == MonitoringType.BOREHOLE_INCLINOMETER) {
            ropeLengthView.setVisibility(View.VISIBLE);
        } else {
    ...
    public boolean updateSensorData(SetVmsTerminalSensorParamsEntity sensorParamsEntity) {
        ...
        //钻孔测斜仪
        if (monitoringType == MonitoringType.BOREHOLE_INCLINOMETER) {
            sensorParamsEntity.setRopelen(null);
        }
    ...
    private boolean checkValueValid() {
        ...
        ropeLength = mEtRopeLength.getText().toString();
